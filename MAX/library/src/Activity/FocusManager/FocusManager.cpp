...
void FocusManager::modifyContentType(
    ...
    if (!channel) {
        LX(ERROR, "modifyContentTypeFailed,reason:channelNotFound,channel:" + channelName);
        return;
    }
    ...
    if (contentType == activity->getContentType()) {
        LX(ERROR, "no contentType to modify it is already identical: " + contentTypeToString(contentType));
        return;
    }
