...
#include "Utils/Threading/WorkerThread.h"
...
static std::string createMoniker() {
    static int number = 1;
    std::string moniker = "multiAgentExperience::workerThread::";
    return moniker.append(std::to_string(number));
}

WorkerThread::WorkerThread() : m_moniker{createMoniker()}, m_stop{false}, m_cancel{false} {
