...
#ifndef MULTI_AGENT_EXPERIENCE_LIBRARY_INCLUDE_UTILS_TIMING_TIMER_H_
#define MULTI_AGENT_EXPERIENCE_LIBRARY_INCLUDE_UTILS_TIMING_TIMER_H_
...
#include "Utils/Error/FinallyGuard.h"
...
bool Timer::start(
    ...
    if (!activate()) {
        //        logger::acsdkError(logger::LogEntry(Timer::getTag(), "startFailed").d("reason",
        //        "timerAlreadyActive"));
        return false;
    }

    // Lock for m_thread.join and m_thread assignment
...
void Timer::callTask(
    ...
    utils::error::FinallyGuard guard{[this] {
        m_stopping = false;
        m_running = false;
        return;
    }};
...
#endif  // MULTI_AGENT_EXPERIENCE_LIBRARY_INCLUDE_UTILS_TIMING_TIMER_H_
